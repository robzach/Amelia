 ...
 * any values of 0 or 1000 (max) back from the sensor are painted red
...
int[] positions = new int[NUM_OF_SENSORS]; // incoming data positions
int[] barHeight = new int[NUM_OF_SENSORS];
boolean[] graph = new boolean[NUM_OF_SENSORS]; // array to store whether to graph each datum or not
...
void drawGraphLines() {
  for (int i = 0; i<NUM_OF_SENSORS; i++) {
    ...
    if (barHeight[i] == 0 || barHeight[i] == 1000) {
      stroke(255,0,0);
      strokeWeight(3);
    } else {
      stroke(255);
...
void drawLegend() {
  for (int i = 0; i<NUM_OF_SENSORS; i++) {
    ...
    noFill();
    rectMode(CORNERS);
    rect((i+1)*75, 10, 50+((i+1)*75), height-50);
    fill(255);
    text("#" + i, (i+1)*75, height-10);
    text(positions[i], (i+1)*75, height-25);
  }
}
...
void serialEvent (Serial myPort) {
  ...
  for (int i = 0; i<5; i++) {
    ...
    barHeight[i] = (int)map(positions[i], 1000, 0, 0, 200);
  }
}
